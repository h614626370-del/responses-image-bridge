...
图生图输入既可使用 PNG、JPEG、WebP 的 Base64 data URL，也可使用公网 HTTPS 图片 URL。远程图片由中间服务下载、校验后转为无损 WebP，再以二进制 multipart 文件提交给图片编辑接口；转换只改变图片容器和元数据，不重新生成画面内容。私网、环回、保留地址、压缩响应、超过 20 MiB 的图片、超过 4000 万像素的图片和超过三次的重定向会被拒绝。
