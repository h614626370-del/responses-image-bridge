...
详情中还保存有界的脱敏请求格式，用于识别客户端字段与嵌套结构；模型、工具选项和数据类型会保留，提示词、普通字符串、Key、Token、Base64 与图片 URL 会替换为类型和长度占位符。
不记录 Authorization 值、提示词原文、Base64 原文、图片内容或完整上游错误体。
`X-Bridge-Request-Id` 可用于客户报错时定位日志。
服务不持久化图片，处理期间图片和请求在内存中；高并发、大图片和大队列会增加内存占用。
